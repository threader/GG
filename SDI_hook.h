...
/* Includeheader
 ...
 1.16  06.10.06 : added new DISPATCHER() macro and separated it from the
       ...
       22.11.08 : use now offical register to var feature of gcc to support GCC >=4  
*/
...
#define DISPATCHERPROTO(name)  ULONG _call_##name(struct IClass * cl,Object * obj, Msg msg);			\
    SAVEDS ASM ULONG  name()																			\
	{																									\
    REG(a0, register struct IClass * cl); REG(a2, register Object * obj);REG(a1, register Msg msg);     \
	return _call_##name(cl,obj,msg);/* call is gone away by optimizer, so dont use same calling func */ \
    }																									\
	ULONG _call_##name(struct IClass * cl,Object * obj, Msg msg)
